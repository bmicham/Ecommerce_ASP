...
const paginate = (product) => {
    ...
    const numbersOfPages = Math.ceil(product.length / itemsPerPage);

    const newProduct = Array.from({ length: numbersOfPages }, (_, index) => {
...
const displayButtons = (btnContainer, pages, activeIndex) => {
    ...
    btns.push(`<button class="preview-btn">prev</button>`)
